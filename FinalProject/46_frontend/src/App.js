  ...
  const cartItems = cart.map((el) => (
    <div className="row border-top border-bottom" key={el._id}>
      <div className="row main align-items-center">
        ...
        <div className="col">
          ${el.price} <span className="close">&#10005;</span> {el.cartQty} ={" "}
          {(el.price * el.cartQty).toFixed(2)}
        </div>
      </div>
    </div>
  ));
  ...
  function buyItems(cart) {
    cart.forEach((el) => {
      ...
      if (newQuantity > 0) {
        ...
        console.log(
          "Updating id : " + el._id + " to have quantity : " + newQuantity
        );
  ...
  if (page == 0) {
    ...
    return (
      <div>
        ...
        <hr></hr>
        <button type="button" variant="light" onClick={() => validate()}>
          {" "}
          Buy{" "}
        </button>
        <button type="button" variant="light" onClick={() => setPage(0)}>
          {" "}
          Continue Shopping
        </button>
      </div>
          ...
          <div className="row">
            <div className="col-md-8 cart">
              <div className="title">
                {/* <div className="row"> */}
                <div className="col">
                  Thank you {userInfo.fullName} for your order using the card
                  ending in {userInfo.cardNumber.slice(-4)}!<br />
